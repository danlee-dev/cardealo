...
| 법인카드 QR/바코드 생성 | 완료 | 법인카드 결제용 QR/바코드 생성 (corp_{id} 형식) |
...
| 잔액 부족 처리 | 완료 | 잔액 부족 시 모달 표시 (insufficient_limit 오류) |
| 한도 초과 처리 | 완료 | 카드 한도 초과 시 오류 표시 |
| 법인카드 한도 확인 | 완료 | 법인카드 결제 시 개인 한도 + 카드 전체 한도 동시 확인 (insufficient_card_limit 오류) |
| 결제 실패 알림 | 완료 | 잔액 부족, 한도 초과 시 사용자 앱에 실패 알림 전송 |
...
| 법인카드 캐러셀 | 완료 | 법인카드 전용 캐러셀 + 페이지네이션 점 표시 |
...
| 반응형 카드 UI | 완료 | 카드 크기에 따른 텍스트/뱃지 자동 스케일링 |
...
| 슬라이드 네비게이션 | 완료 | 관리자 화면 진입/퇴장 시 부드러운 슬라이드 애니메이션 |
...
| **결제** | POST | /api/qr/generate | QR/바코드 생성 (개인/법인 카드 모두 지원) |
| | GET | /api/qr/scan-status | 스캔 상태 확인 |
| | POST | /api/payment/process | 결제 처리 |
| | POST | /api/payment/webhook | 결제 웹훅 수신 |
| | POST | /api/payment/notify-failure | 결제 실패 알림 (잔액 부족/한도 초과) |
...
| | POST | /api/payment/process | 결제 확정 (잔액/한도 확인 포함) |
| | GET | /api/payment/history | 결제 내역 |
| | POST | /api/corporate/check-limit | 법인카드 한도 확인 (개인/카드 전체 한도) |
